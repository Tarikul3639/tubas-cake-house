...
import { Search, Weight, Utensils, RotateCcw, Box } from "lucide-react";
import { CakeCard } from "@/components/cards/CakeCard";
import { initialCakes } from "@/data/cakes";

// 1. This is our new inner component where useSearchParams will be used
  ...
  const filteredCakes = useMemo(() => {
    ...
    if (selectedCategory !== "all") {
      result = result.filter(
        (cake) => cake.category.toLowerCase() === selectedCategory.toLowerCase()
      );
    }
    if (searchQuery) {
    ...
    return result;
  }, [searchQuery, priceRange, sortBy, selectedCategory, selectedWeight]);

  // Your current JSX to return will be here
  return (
      ...
      <header className="mb-12 flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <h1 className="text-4xl md:text-6xl font-black text-slate-800 italic mb-2">
            {selectedCategory === "all" ? "Nexion" : selectedCategory}{" "}
            <span className="text-pink-500">Cakes</span>
          </h1>
          <p className="text-slate-500 font-medium tracking-tight">
            Choose from our signature and custom collections.
          </p>
        </div>
        <button
          onClick={() => {
        ...
        <aside className="w-full lg:w-72 space-y-6">
          ...
          <div className="bg-white p-6 rounded-[32px] border border-pink-50 shadow-sm">
            ...
            <div className="space-y-2">
              {["all", "Custom", "Bestseller", "Trending", "Classic"].map(
                (cat) => (
                  <button
                    key={cat}
                    ...
                    className={`flex items-center justify-between w-full px-4 py-3 rounded-2xl text-xs font-black uppercase tracking-wider transition-all ${
                      selectedCategory === cat
                        ...
                        : "bg-slate-50 text-slate-500 hover:bg-pink-50"
                    }`}
                  >
                    {cat}
                    {cat === "Custom" && (
          ...
          <div className="bg-white p-6 rounded-[32px] border border-pink-50 shadow-sm">
            <div className="flex justify-between items-center mb-6">
              <h4 className="font-black text-slate-800 uppercase text-[10px] tracking-[0.2em]">
                Max Price
              </h4>
              <span className="bg-pink-50 text-pink-500 px-3 py-1 rounded-lg font-black text-sm">
                ${priceRange}
              </span>
            </div>
            <input
                ...
                <button
                  ...
                  className={`py-3 rounded-xl text-[10px] font-black uppercase transition-all border ${
                    selectedWeight === w
                      ...
                      : "bg-transparent text-slate-400 border-slate-100 hover:border-pink-200"
                  }`}
                >
                  {w}
                </button>
          ...
          <AnimatePresence mode="popLayout">
            {filteredCakes.length > 0 ? (
              ...
              <motion.div
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                className="text-center py-32 bg-white rounded-[48px] border-2 border-dashed border-pink-100"
              >
                <div className="w-20 h-20 bg-pink-50 rounded-full flex items-center justify-center mx-auto mb-6">
                  <Box className="text-pink-200" size={32} />
                </div>
                <p className="text-slate-400 font-black uppercase text-xs tracking-widest">
                  No cakes found in this filter.
                </p>
              </motion.div>
            )}
          </AnimatePresence>
...
// 2. Main export function wrapped with Suspense
  ...
  return (
    <main className="min-h-screen bg-[#fffafb] pt-28 pb-20">
      <Suspense
        fallback={
          <div className="container mx-auto px-6">
            <div className="mb-12 h-20 w-1/3 bg-pink-50 animate-pulse rounded-2xl" />{" "}
            {/* Title Skeleton */}
            <div className="flex gap-8">
              <div className="hidden lg:block w-72 h-125 bg-pink-50 animate-pulse rounded-[32px]" />{" "}
              {/* Sidebar Skeleton */}
              <div className="flex-1">
                {/* <LoadingCakesGrid />  */}
              </div>
